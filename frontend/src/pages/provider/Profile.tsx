...
import { getProfile, updateProfile } from '../../api/provider';
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
        <div className="flex items-center gap-5">
          <div className="w-20 h-20 bg-yellow-600 rounded-full flex items-center justify-center text-white font-bold text-3xl">
            {form.name?.charAt(0)?.toUpperCase() ?? '🚜'}
          </div>
          <div>
            <h2 className="text-xl font-bold text-gray-800">{form.name || 'Provider'}</h2>
            <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-xs font-semibold mt-1">
              🚜 PROVIDER
            </span>
