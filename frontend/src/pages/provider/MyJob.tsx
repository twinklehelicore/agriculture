...
import { getAssignedRequests, approveRequest, startProgress, completeRequest, rejectRequest } from '../../api/provider';
...
const nextAction: Record<string, { label: string; color: string }> = {
  ASSIGNED: { label: 'Approve', color: 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100' },
  APPROVED: { label: 'Start Work', color: 'bg-purple-50 text-purple-700 hover:bg-purple-100' },
  IN_PROGRESS: { label: 'Mark Complete', color: 'bg-green-50 text-green-700 hover:bg-green-100' },
};
...
export default function MyJobs() {
  ...
  const statuses = ['ALL', 'ASSIGNED', 'APPROVED', 'IN_PROGRESS', 'COMPLETED', 'REJECTED'];
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Manage your assigned service requests</p>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setRejecting(null)} className="text-gray-400 hover:text-gray-600 text-xl">✕</button>
            </div>
            <div className="px-4 py-3 bg-red-50 border border-red-200 rounded-xl">
              <p className="text-sm text-red-700">
                Are you sure you want to reject <span className="font-semibold">Job #{rejecting}</span>?
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {['#', 'Farmer ID', 'Farm ID', 'Slot', 'Details', 'Status', 'Applied', 'Actions'].map(h => (
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.length === 0 ? (
                  ...
                  <tr key={job.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-700">#{job.farmerId}</td>
                    <td className="px-4 py-3 text-gray-700">#{job.farmId}</td>
                    <td className="px-4 py-3 text-gray-600">{job.preferredSlot ?? '—'}</td>
                    <td className="px-4 py-3 text-gray-600 max-w-[140px] truncate">{job.details ?? '—'}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${statusColor[job.status] ?? 'bg-gray-100 text-gray-600'}`}>
                      ...
                      <div className="flex gap-2">
                        {nextAction[job.status] && (
                          <button
                            ...
                            {actionLoading === job.id ? '...' : nextAction[job.status].label}
                          </button>
                        )}
                        {job.status === 'ASSIGNED' && (
                          <button onClick={() => setRejecting(job.id)}
