...
const serviceRequest = async (req: Request, res: Response) => {
  try {
    ...
    await prisma.notification.createMany({
      data: [
        { userId: adminId!, title: 'New Service Request', message: `A new ${service.name} request has been created by farmer #${userId}.` },
      ]
    });
...
const deleteServiceRequest = async (req: Request, res: Response) => {
  try {
    ...
    await prisma.serviceRequest.delete({
      where: { id: Number(id), farmerId: userId },
    });
...
export default { createFarm, updateFarm, listFarm, getFarmById, serviceRequest, listMyServiceRequest, deleteServiceRequest, deleteFarm, listCrop, listServices, listCategories
};
